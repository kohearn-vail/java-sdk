...
public class ConferencesSearchFilters extends Filters {
	...
	/**
	 * Retrieve the value of the dateCreated filter as a Java {@link java.util.Date}
	 * object.
	 *
	 * @return The date a conference in the results must have been created.
	...
	public Date getDateCreatedAsDate() throws FreeClimbException {
		...
		try {
			...
			if (pos == null) {
				...
				throw new FreeClimbException("dateCreated field in ConferencesRequestFilters was null", npe);
			}
		}
		return dc;
	}
	...
	/**
	 * Retrieve the value of the dateUpdated filter as a Java {@link java.util.Date}
	 * object.
	 *
	 * @return The date a conference in the results must have been updated.
	 * @throws FreeClimbException when the date is null or can't be parsed.
	 */
	public Date getDateUpdatedAsDate() throws FreeClimbException {
		ParsePosition pos = new ParsePosition(0);
		...
		try {
			...
			if (pos.getErrorIndex() >= 0) {
				throw new FreeClimbDateException(this.dateUpdated, pos.getErrorIndex());
			}
		} catch (NullPointerException npe) {
			if (pos == null) {
				throw new FreeClimbSDKException(npe);
			} else {
				throw new FreeClimbException("dateUpdated field in ConferencesRequestFilters was null", npe);
			}
		}
		return dc;
	}
	...
	/**
	 * Set the status filter for conferences that are in the given state / have the
	 * given status.
	 * 
	...
	/**
	 * Set the dateCreated filter to find conferences that were created on that
	 * date.
	 * 
	...
	/**
	 * Set the dateUpdated filter to find conferences that were updated on that
	 * date.
	 * 
