...
public class AvailablePhoneNumberRequesterTest {
    ...
    private static String numberList = "{\"total\": 1, \"start\": 0, \"end\": 0, \"page\": 0, \"numPages\": 1, \"pageSize\": 1, \"nextPageUri\": null, \"availablePhoneNumbers\": ["
    ...
    public void getList() throws FreeClimbException {
        Helper.getMockServer().when(request().withMethod("GET").withPath(requester.getPath()))
                .respond(response().withStatusCode(200).withBody(AvailablePhoneNumberRequesterTest.numberList));
    ...
    public void getListFiltered() throws Throwable {
        ...
        Helper.getMockServer()
                .when(request().withMethod("GET").withPath(requester.getPath()).withQueryStringParameters(listQuery))
                .respond(response().withStatusCode(200).withBody(AvailablePhoneNumberRequesterTest.numberList));
