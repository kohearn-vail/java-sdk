...
public class CallsRequesterTest {
	...
	private static String aTestCallList = "{\"total\": 4, \"start\": 0, \"end\": 0, \"page\": 0, \"numPages\": 1, \"pageSize\": 20, \"nextPageUri\": null, \"calls\": ["
			+ CallsRequesterTest.aTestCall + "," + CallsRequesterTest.aTestCall + "," + CallsRequesterTest.aTestCall
			+ "," + CallsRequesterTest.aTestCall + "]}";
	...
	public void getCallsList() throws Throwable {
		Helper.getMockServer().when(request().withMethod("GET").withPath(this.callsR.getPath()))
				.respond(response().withStatusCode(200).withBody(CallsRequesterTest.aTestCallList));
	...
	public void getCallsListWithFilters() throws Throwable {
		...
		Helper.getMockServer()
				.when(request().withMethod("GET").withPath(this.callsR.getPath()).withQueryStringParameters(listQuery))
				.respond(response().withStatusCode(200).withBody(CallsRequesterTest.aTestCallList));
	...
	public void getCallWithCallId() throws Throwable {
		...
		Helper.getMockServer().when(request().withMethod("GET").withPath(this.callsR.getPath() + "/" + callId))
				.respond(response().withStatusCode(200).withBody(CallsRequesterTest.aTestCallList));
	...
	public void updateCall(String callId, String status) throws Throwable {
		...
		Helper.getMockServer().verify(request().withMethod("POST").withPath(this.callsR.getPath() + "/" + callId)
