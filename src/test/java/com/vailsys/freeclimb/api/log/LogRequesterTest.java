...
public class LogRequesterTest {
    ...
    private String listJson = "{\"total\":1, \"start\":0, \"end\":0, \"page\":0,\"numPages\":1,\"pageSize\":1, \"nextPageUri\":null, \"logs\": ["
            + this.logJson + "]}";
    ...
    public void getFilteredList() throws Throwable {
        ...
        Helper.getMockServer()
                .when(request().withMethod("POST").withPath(requester.getPath()).withBody(gson.toJson(filters)))
                .respond(response().withStatusCode(200).withBody(listJson));
    ...
    public void getFilteredList(String requestId) throws Throwable {
        ...
        Helper.getMockServer()
                .verify(request().withMethod("POST").withPath(requester.getPath())
                        .withBody(new JsonBody("{pql:'" + filters.getPql() + "',requestId:'" + requestId + "'}")),
