...
public class RecordingsRequesterTest {
	...
	private static final String aTestRecordingList = "{\"total\": 4, \"start\": 0, \"end\": 0, \"page\": 0, \"numPages\": 1, \"pageSize\": 20, \"nextPageUri\": null, \"recordings\": ["
			+ RecordingsRequesterTest.aTestRecording + "," + RecordingsRequesterTest.aTestRecording + ","
			+ RecordingsRequesterTest.aTestRecording + "," + RecordingsRequesterTest.aTestRecording + "]}";
	...
	public void canDeleteRecording(String recordingId) throws Throwable {
		Helper.getMockServer()
				.when(request().withMethod("DELETE").withPath(this.recordingsR.getPath() + "/" + recordingId))
	...
	public void canGetRecording(String recordingId) throws Throwable {
		Helper.getMockServer()
				.when(request().withMethod("GET").withPath(this.recordingsR.getPath() + "/" + recordingId))
				.respond(response().withStatusCode(200).withBody(RecordingsRequesterTest.aTestRecording));

		assertTrue(this.recordingsR.getMetaByRecordingId(recordingId)
				.equals(Recording.fromJson(RecordingsRequesterTest.aTestRecording)));
	}

	@Then("^get a list of all recordings$")
	public void canGetRecordingsMeta() throws Throwable {
		Helper.getMockServer().when(request().withMethod("GET").withPath(this.recordingsR.getPath()))
				.respond(response().withStatusCode(200).withBody(RecordingsRequesterTest.aTestRecordingList));
	...
	public void canGetFilteredRecordingsMeta() throws Throwable {
		...
		Helper.getMockServer()
				.when(request().withMethod("GET").withPath(this.recordingsR.getPath())
						.withQueryStringParameters(listQuery))
				.respond(response().withStatusCode(200).withBody(RecordingsRequesterTest.aTestRecordingList));
	...
	public void downloadToFileWithPath(String recordingId) throws Throwable {
		Helper.getMockServer()
				.when(request().withMethod("GET")
						.withPath(this.recordingsR.getPath() + "/" + recordingId + "/Download"))
				.respond(response().withStatusCode(200).withBody(RecordingsRequesterTest.testRecordingFile));
	...
	public void streamRecording(String recordingId) throws Throwable {
		Helper.getMockServer()
				.when(request().withMethod("GET").withPath(this.recordingsR.getPath() + "/" + recordingId + "/Stream"))
				.respond(response().withStatusCode(200).withBody(RecordingsRequesterTest.testRecordingFile));
