...
public class RecordingsSearchFiltersTest {
	...
	public void checkAllFieldsAreNull() throws Throwable {
		...
		} catch (FreeClimbSDKException pse) {
			fail("getDateCreatedAsDate should have failed with a FreeClimbException but failed with "
					+ pse.getMessage());
			return;
		} catch (FreeClimbDateException pde) {
			fail("getDateCreatedAsDate should have failed with a FreeClimbException but failed with "
					+ pde.getMessage());
			return;
		...
		fail("getDateCreatedAsDate should have failed with a FreeClimbException");
	}
	...
	public void checkGetDateCreated(String dateCreated) throws FreeClimbException {
		ParsePosition pos = new ParsePosition(0);
		Date checkDate = RecordingsSearchFilters.dateCreatedFormat.parse(dateCreated, pos);
		assertTrue(pos.getErrorIndex() < 0);
